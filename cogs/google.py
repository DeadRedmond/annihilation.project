...
class Google:
    ...
    @commands.command(pass_context=True, aliases=['image', 'img', 'картинка'])
    async def i(self, ctx, *, query):
        ...
        if query[0].isdigit():
            ...
            item = 0
        async with self.bot.session.get("https://www.googleapis.com/customsearch/v1?q=" + urllib.parse.quote_plus(query) + "&start=" + '1' + "&key=" + google_api_key + "&cx=" + custom_search_engine + "&searchType=image") as resp:
            if resp.status != 200:
                if not google_api_key or not custom_search_engine:
